//! Where Accelerate keeps its files.
//!
...
pub fn config_dir() -> PathBuf {
    if let Some(override_dir) = std::env::var_os("ACCELERATE_CONFIG_DIR") {
        return PathBuf::from(override_dir);
    }

    if cfg!(target_os = "macos") {
        home().join("Library/Application Support/Accelerate")
    } else {
...
pub fn data_dir() -> PathBuf {
    if let Some(override_dir) = std::env::var_os("ACCELERATE_DATA_DIR") {
        return PathBuf::from(override_dir);
    }
