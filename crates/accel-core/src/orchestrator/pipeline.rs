...
mod tests {
    ...
    impl FakeWorkspace {
        fn with_changes() -> Self {
            Self {
                path: PathBuf::from("/tmp/accel-fake"),
    ...
    async fn a_run_that_never_satisfies_review_is_handed_to_a_human() {
        ...
        assert!(board.blockers_set()[0].starts_with("Accelerate:"));
    ...
    async fn stopping_a_run_leaves_the_task_ready_again() {
        ...
        assert_eq!(board.blockers_set(), vec!["Accelerate: stopped by request"]);
    }
    ...
    async fn a_run_cannot_start_without_an_implementer() {
        ...
        assert!(board.blockers_set()[0].contains("Accelerate"));
    }
