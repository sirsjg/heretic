...
//! deliberately permissive (`Option`, `#[serde(default)]`) because Accelerate must
...
///
/// Flux enforces one transition rule that Accelerate must respect: a task in
...
pub struct Epic {
    ...
    #[serde(default)]
    pub notes: String,
    /// The gate Accelerate honours: only epics with `auto = true` are worked
