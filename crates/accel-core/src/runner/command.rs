...
/// The provider id Accelerate declares when pointing Codex at a host of our
/// own. Deliberately not one of Codex's built-in ids, which it refuses to let
/// configuration override.
const OSS_PROVIDER: &str = "accelerate-oss";
            ...
            if let RunnerKind::CodexOss { base_url } = &profile.runner {
                match base_url.as_deref().filter(|url| !url.is_empty()) {
                    ...
                    Some(url) => {
                        args.push("-c".into());
                        args.push(format!(
                            "model_providers.{OSS_PROVIDER}.name=\"Accelerate host\""
                        ));
                        args.push("-c".into());
                        args.push(format!(
...
mod tests {
    ...
    fn a_model_on_another_machine_is_reached_through_a_declared_provider() {
        ...
        assert!(
            joined.contains(
                "model_providers.accelerate-oss.base_url=\"http://spark.local:11434/v1\""
            ),
            "{joined}"
        );
        // Current Codex accepts only the responses wire format from a custom
        // provider; "chat" is rejected at config load.
        assert!(joined.contains("model_providers.accelerate-oss.wire_api=\"responses\""));
        assert!(joined.contains("model_provider=\"accelerate-oss\""));
    ...
    fn display_keeps_configuration_arguments_legible() {
        ...
        assert!(
            shown.contains("model_provider=\"accelerate-oss\""),
            "{shown}"
        );
    }
}
