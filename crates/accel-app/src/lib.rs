...
//! `accel-core`, which is why it can be tested without a GUI.
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "accel_app=info,accel_core=info".into()),
        )
        .init();
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            tauri::async_runtime::spawn(async move {
                ...
                let watcher = accel_core::FluxWatcher::start(settings.flux.clone());
                ...
                loop {
                    match events.recv().await {
                        Ok(event) => {
                            ...
                            if matches!(
                                event,
                                accel_core::FluxEvent::Changed(_)
                                    | accel_core::FluxEvent::Invalidated
                            ) {
