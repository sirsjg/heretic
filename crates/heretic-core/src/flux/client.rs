...
pub enum FluxError {
    ...
    #[error("flux returned a response Heretic could not parse: {0}")]
    Decode(String),
...
impl FluxClient {
    ...
    /// Flip an epic's `auto` flag — the switch that decides whether Heretic may
    ...
    /// Heretic writes one at every meaningful step of a run.
...
pub(crate) fn detect_proxy_challenge(
    ...
    Some(format!(
        "{who} answered with a sign-in page instead of Flux (HTTP {status}). Heretic is not \
