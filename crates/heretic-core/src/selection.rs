...
//!
//! Autonomy in Heretic is opt-in and narrow: a task is only eligible when the
...
pub enum Ineligible {
    ...
    ExternallyBlocked,
    Archived,
    /// Heretic is already running this task.
    AlreadyRunning,
}
...
impl<'a> BoardSnapshot<'a> {
    ...
    /// Judge a single task. `running` holds task ids Heretic already has in flight.
